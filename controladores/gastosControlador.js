 ...
 */
/**
 * 1. Crear nuevo gasto (con notificaciones)
 */
export const crearGasto = async (usuarioId, datosGasto) => {
  ...
  try {
    ...
    console.log('✅ Gasto creado exitosamente ID:', nuevoGasto.id);

    // ========== NOTIFICACIONES ==========
    try {
      // Notificar a todos los familiares (excepto al creador)
      const mensaje = `Nuevo gasto "${descripcion}" por $${monto} creado`;
      await notificarAFamiliares(
        ...
        'gastos',
        usuarioId,
        usuarioId // excluye al creador
      );

      // Notificar al responsable si existe y no es el creador
      if (responsableId && responsableId !== usuarioId) {
        await crearNotificacion({
          usuario_id: responsableId,
          tipo: 'gasto_asignado',
          mensaje: `Te han asignado el gasto "${descripcion}" por $${monto}`,
          referencia_id: nuevoGasto.id,
          referencia_tipo: 'gastos',
          emisor_id: usuarioId
        });
      }
    } catch (notifError) {
      console.error('⚠️ Error al enviar notificaciones de creación:', notifError.message);
...
/**
 * 6. Actualizar gasto (con notificaciones)
 */
export const actualizarGasto = async (gastoId, usuarioId, datosActualizacion) => {
  ...
  try {
    ...
    // 1. Obtener gasto actual (para comparar)
    const gastoActualQuery = `
      SELECT g.*, am.id as adulto_mayor_id
      FROM gastos g
      LEFT JOIN adultos_mayores am ON g.adulto_mayor_id = am.id
      WHERE g.id = $1 AND g.deleted_at IS NULL
    `;
    const gastoActualResult = await client.query(gastoActualQuery, [gastoId]);
    if (gastoActualResult.rows.length === 0) {
      return {
        exito: false,
    ...
    const gastoActual = gastoActualResult.rows[0];
    const adulto_mayor_id = gastoActual.adulto_mayor_id;
    ...
    try {
      ...
      if (datosActualizacion.responsable_id &&
        datosActualizacion.responsable_id !== gastoActual.responsable_id) {
        if (datosActualizacion.responsable_id !== usuarioId) {
          await crearNotificacion({
            usuario_id: datosActualizacion.responsable_id,
            tipo: 'gasto_asignado',
            mensaje: `Te han asignado el gasto "${gastoActualizado.descripcion}" por $${gastoActualizado.monto}`,
      ...
      if (datosActualizacion.estado === 'pagado' && gastoActual.estado !== 'pagado') {
        await notificarAFamiliares(
          adulto_mayor_id,
          'gasto_pagado',
          `El gasto "${gastoActualizado.descripcion}" ha sido pagado`,
          gastoId,
          'gastos',
          usuarioId,
          usuarioId
      ...
      console.error('⚠️ Error al enviar notificaciones de actualización:', notifError.message);
    }
...
/**
 * 8. Marcar gasto como pagado (con notificaciones)
 */
export const marcarGastoPagado = async (gastoId, usuarioId) => {
  ...
  try {
    ...
    const verificarQuery = `
      ...
      LEFT JOIN adultos_mayores am ON g.adulto_mayor_id = am.id
      WHERE g.id = $1 
      AND (g.creado_por = $2 OR g.adulto_mayor_id IN (
        SELECT adulto_mayor_id FROM familiares WHERE usuario_id = $2
      ))
      AND g.deleted_at IS NULL
    `;
    ...
    const gasto = verificarResult.rows[0];
    const adulto_mayor_id = gasto.adulto_mayor_id;
    ...
    const result = await client.query(query, [gastoId]);
    const gastoPagado = result.rows[0];

    console.log('✅ Gasto marcado como pagado:', gastoPagado.descripcion);
    ...
    try {
      await notificarAFamiliares(
        adulto_mayor_id,
        'gasto_pagado',
        `El gasto "${gastoPagado.descripcion}" ha sido marcado como pagado`,
        gastoId,
        'gastos',
        usuarioId,
        usuarioId
      ...
      console.error('⚠️ Error al notificar pago:', notifError.message);
    }
...
/**
 * 12. Registrar aporte a gasto (con notificaciones)
 */
export const registrarAporteGasto = async (gastoId, usuarioId, monto, notas = '') => {
  ...
  try {
    ...
    const usuarioValidoResult = await client.query(usuarioValidoQuery, [usuarioId, adulto_mayor_id]);
    ...
    // ========== NOTIFICACIONES ==========
    try {
      // Obtener datos del usuario que aporta
      const usuarioQuery = `SELECT nombre, apellido FROM usuarios WHERE id = $1`;
      const usuarioRes = await client.query(usuarioQuery, [usuarioId]);
      const usuarioNombre = usuarioRes.rows.length
        ? `${usuarioRes.rows[0].nombre || ''} ${usuarioRes.rows[0].apellido || ''}`.trim() || 'Un familiar'
        : 'Un familiar';

      // Notificar al creador del gasto (si no es el mismo)
      if (gasto.creado_por && gasto.creado_por !== usuarioId) {
        await crearNotificacion({
          usuario_id: gasto.creado_por,
          tipo: 'aporte_gasto',
          mensaje: `${usuarioNombre} ha aportado $${montoAporte} al gasto "${gasto.descripcion}"`,
          referencia_id: gastoId,
          referencia_tipo: 'gastos',
          emisor_id: usuarioId
        });
      ...
      // Si hay responsable y es diferente al creador y al que aporta
      if (gasto.responsable_id &&
        gasto.responsable_id !== usuarioId &&
        gasto.responsable_id !== gasto.creado_por) {
        await crearNotificacion({
          usuario_id: gasto.responsable_id,
          tipo: 'aporte_gasto',
          mensaje: `${usuarioNombre} ha aportado $${montoAporte} al gasto "${gasto.descripcion}"`,
          referencia_id: gastoId,
          referencia_tipo: 'gastos',
      ...
      // Si el gasto quedó cubierto, notificar a todos los familiares
      if (gastoCubierto) {
        await notificarAFamiliares(
          adulto_mayor_id,
          'gasto_pagado',
          `El gasto "${gasto.descripcion}" ha sido completamente cubierto con el aporte de ${usuarioNombre}`,
          gastoId,
          'gastos',
          usuarioId,
          usuarioId
      ...
      console.error('⚠️ Error al notificar aporte:', notifError.message);
    }
