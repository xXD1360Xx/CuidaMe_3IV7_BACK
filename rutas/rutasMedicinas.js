...
router.post('/todas', autenticarUsuario, async (req, res) => {
  try {
    ...
    if (!usuario_id) {
      return res.status(400).json({ exito: false, error: 'ID de usuario es requerido', codigo: 'USUARIO_ID_REQUERIDO' });
    }
    ...
    res.status(500).json({ exito: false, error: 'Error interno del servidor', codigo: 'ERROR_INTERNO' });
  }
});
...
router.post('/frecuentes', autenticarUsuario, async (req, res) => {
  try {
    ...
    const resultado = await medicinasControlador.obtenerMedicinasFrecuentes(usuario_id, limite || 5);
    res.status(200).json(resultado);
  } catch (error) {
...
router.post('/crear', autenticarUsuario, async (req, res) => {
  try {
    ...
    if (!usuario_id || !medicina) {
      return res.status(400).json({ exito: false, error: 'ID de usuario y datos de medicina son requeridos', codigo: 'DATOS_INCOMPLETOS' });
    }
...
router.post('/actualizar', autenticarUsuario, async (req, res) => {
  try {
    ...
    if (!usuario_id || !medicina_id) {
      return res.status(400).json({ exito: false, error: 'ID de usuario y medicina son requeridos', codigo: 'DATOS_INCOMPLETOS' });
    }
    const resultado = await medicinasControlador.actualizarMedicina(medicina_id, usuario_id, medicina);
    if (!resultado.exito) {
...
router.post('/marcar-tomada', autenticarUsuario, async (req, res) => {
  try {
    const { medicina_id, fecha_toma, horario, observaciones } = req.body;
    const usuario_id = req.usuario.id; // ✅ obtener del token

    if (!medicina_id || !horario) {
      return res.status(400).json({ exito: false, error: 'ID de medicina y horario son requeridos', codigo: 'DATOS_INCOMPLETOS' });
    }
...
router.post('/registros', autenticarUsuario, async (req, res) => {
  try {
    ...
    const resultado = await medicinasControlador.obtenerRegistrosMedicina(usuario_id, fecha_inicio, fecha_fin);
    res.status(200).json(resultado);
  } catch (error) {
...
router.post('/por-frecuencia', autenticarUsuario, async (req, res) => {
  try {
    ...
    if (!usuario_id || !frecuencia) {
      return res.status(400).json({ exito: false, error: 'ID de usuario y frecuencia son requeridos', codigo: 'DATOS_INCOMPLETOS' });
    }
    const resultado = await medicinasControlador.obtenerMedicinasPorFrecuencia(usuario_id, frecuencia);
    res.status(200).json(resultado);
  } catch (error) {
...
router.post('/actualizar-stock', autenticarUsuario, async (req, res) => {
  try {
    const { medicina_id, nuevo_stock } = req.body;
    const usuario_id = req.usuario.id; // ✅ obtener del token

    if (!medicina_id || nuevo_stock === undefined) {
      return res.status(400).json({ exito: false, error: 'Todos los campos son requeridos', codigo: 'DATOS_INCOMPLETOS' });
    }

    const resultado = await medicinasControlador.actualizarStockMedicina(medicina_id, usuario_id, nuevo_stock);
...
router.post('/buscar', autenticarUsuario, async (req, res) => {
  try {
    ...
    if (!busqueda || busqueda.trim().length < 2) {
      return res.status(200).json({ exito: true, medicinas: [], busqueda: busqueda || '', mensaje: 'Término de búsqueda muy corto' });
    }
...
router.post('/horarios-hoy', autenticarUsuario, async (req, res) => {
  try {
    ...
    const medicinasConHorarios = resultado.medicinas.map(medicina => {
      ...
      return { ...medicina, horarios_detalle: horariosDetalle };
    });
    res.status(200).json({
      exito: true,
...
router.post('/resumen-diario', autenticarUsuario, async (req, res) => {
  try {
    ...
    const medicinasParaDia = todasResult.medicinas.filter(medicina => {
      ...
      if (medicina.frecuencia === 'semanal' && medicina.dias_semana?.includes(diaSemana)) return true;
      if (medicina.frecuencia === 'fecha_especifica') {
        ...
        return fechaActual >= inicio && (!fin || fechaActual <= fin);
      }
      return false;
    });
    const registrosResult = await medicinasControlador.obtenerRegistrosMedicina(usuario_id, fechaConsulta, fechaConsulta);
    ...
    const porcentajeCumplimiento = totalMedicinas > 0 ? Math.round((medicinasTomadas / totalMedicinas) * 100) : 0;
    const porHorario = {
  ...
  try {
    ...
    const reporte = {
      ...
      cumplimiento: {
        ...
        porcentaje_completado: registros.length > 0 ? Math.round((registros.filter(r => r.completada).length / registros.length) * 100) : 0
      },
      alertas: {
        ...
        try {
          switch (cambio.tipo) {
            case 'crear_medicina':
              ...
              break;
            case 'marcar_tomada':
              await medicinasControlador.marcarMedicinaTomada(cambio.medicina_id, usuario_id, cambio.datos);
              cambiosProcesados++;
              break;
            case 'actualizar_stock':
              await medicinasControlador.actualizarStockMedicina(cambio.medicina_id, usuario_id, cambio.nuevo_stock);
              cambiosProcesados++;
              break;
            default:
